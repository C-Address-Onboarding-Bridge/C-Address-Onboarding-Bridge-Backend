...
export const ko: MessageCatalog = {
  ...
  'error.request_failed':              '요청이 실패했습니다 (상태 코드: {{status}}).',

  'error.auth.unauthorized':           '인증되지 않았습니다. API 키를 확인하세요.',
  'error.auth.forbidden':              '접근이 금지되었습니다. 이 작업을 수행할 권한이 없습니다.',
  ...
  'error.validation.missing_field':    '필수 필드가 누락되었습니다: {{field}}.',
  ...
  'error.invalid_c_address':           '"{{address}}"은(는) 유효한 C-주소(Soroban 스마트 계정)가 아닙니다.',
  'error.invalid_g_address':           '"{{address}}"은(는) 유효한 G-주소(클래식 Stellar 계정)가 아닙니다.',
